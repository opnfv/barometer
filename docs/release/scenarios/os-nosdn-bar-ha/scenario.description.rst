...
on compute nodes so that its statistics, events and alarming services can be relayed to Gnoochi and Aodh.
...
Also, verify if plugins/services are communicating with Gnocchi and Aodh on controller nodes.
...
After installation, plugins will be able to read/write the stats on/from controller node.
A detailed list of supported plugins along with their sample configuration can be found in userguide.
