...
a Kafka Broker. The VES message formatting application, ves_app.py, recieves metrics from
...
The VES application can be run in native mode (baremetal), host mode (within a hypervisor) or guest
...
1. Dependencies: install JAVA & Zookeeper.
   ...
   CentOS:
    ...
    .. code:: bash
        ...
        $ sudo yum install java-1.7.0-openjdk epel-release
        $ sudo yum install python-pip zookeeper
...
- On Host with VMs: `Setup VES application (host mode)`_
- Native: `Setup VES application (native mode)`_
...
Create /opt/collectd/etc/collectd.conf collectd configuration file as following
...
Setup VES application (native mode)
--------------------------
...
.. code:: bash
    ...
    $ git clone https://gerrit.opnfv.org/gerrit/barometer ~/barometer
    $ cd ~/barometer/3rd_party/collectd-ves-app/ves_app
    $ nohup python ves_app.py --events-schema=native.yaml --config=ves_app_config.conf > ves_app.stdout.log &


.. figure:: ves-app-native-mode.png

    VES Native mode setup

Create /opt/collectd/etc/collectd.conf collectd configuration file as following

.. include:: collectd-ves-native.conf
                    :code: bash
