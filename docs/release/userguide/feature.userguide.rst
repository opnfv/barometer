...
* The `Exec plugin`_ allows the execution of external programs but counters the security
  concerns by ensuring that only one instance of the program is executed by collectd at any time
  and by forcing the plugin to check that custom programs are never executed with superuser 
  privileges.
