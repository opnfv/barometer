...
NOTIFICATION_FILE = '/tmp/python-notifications.dump'
...
class ConfigServer(object):
    ...
    def __init__(self, host, user, logger, priv_key=None):
        ...
        while (attempt <= 10) and not fuel_node_passed:
            stdin, stdout, stderr = ssh.exec_command("source stackrc; nova list")
	...
	print self.__nodes[0]._Node__ip
    	return [node for node in self.__nodes if 'controller' in node.get_name()]
    def get_computes(self):
        #Get list of computes
        return [node for node in self.__nodes if 'compute' in node.get_name()]
    def get_nodes(self):
        #Get list of nodes
        return self.__nodes
    ...
    def get_plugin_interval(self, compute, plugin):
        ...
        ssh, sftp = self.__open_sftp_session(compute.get_ip(), 'root', 'opnfvapex')
        in_plugin = False
        plugin_name = ''
    ...
    def is_gnocchi_running(self, controller):
        ...
        lines = self.execute_command(
            'source overcloudrc.v3;openstack service list | grep gnocchi', controller.get_ip())
        for line in lines: 
            if 'gnocchi' in line:
    ...
    def is_installed(self, compute, package):
        ...
        stdout = self.execute_command(
            'yum list installed | grep {}'.format(package), compute.get_ip())
        return len(stdout) > 0
    ...
    def enable_ovs_events(
             self, compute, plugins, error_plugins, create_backup=True):
        """Enable OVS Events plugin on compute node
        ...
        """
        try:
            f = sftp.open(COLLECTD_NOTIFICATION,'w+')
        except IOError:
            self.__logger.warning(
                'Cannot open {} on node {}'.format(
                    COLLECTD_NOTIFICATION, compute.get_name()))
            return False
        f.write("import collectd \n")
        f.write("def notify(n):")
        f.write("\n")
        f.write("    f = open('/tmp/python-notifications.dump', 'a')")
        f.write("\n")
        f.write("    f.write('host: {}"\n"'.format(n.host))")
        f.write("\n")
        f.write("    f.write('plugin: {}"\n"'.format(n.plugin))")
        f.write("\n")
        f.write("    f.write('plugin_instance: {}"\n"'.format(n.plugin_instance))")
        f.write("\n")
        f.write("    f.write('type: {}"\n"'.format(n.type))")
        f.write("\n")
        f.write("    f.write('type_instance: {}"\n"'.format(n.type_instance))")
        f.write("\n")
        f.write("    f.write('time: {}"\n"'.format(n.time))")
        f.write("\n")
        f.write("    f.write('severity: {}"\n"'.format(n.severity))")
        f.write("\n")
        f.write("    f.write('message: {}"\n"'.format(n.message))")
        f.write("    f.write("\n")")
        f.write("    f.close() "\n"")
        f.write("\n")
        f.write("collectd.register_notification(notify)")
        f.close()
        """

        #Step-2 Modify the collectd.conf file
        plugins = ['python']
        if ConfigServer.enable_plugins(self, compute, plugins, error_plugins, create_backup=False):
            self.__logger.info("Python plugin enabled on the compute node {}" .format(compute.get_name()))
        else:
            self.__logger.info("Failed to enable python plugin on compute node {}".format(compute.get_name()))
            return False

        #Step-4 Create a bridge on compute node
        ssh, sftp = self.__open_sftp_session(compute.get_ip(), 'root', 'opnfvapex')
        stdout = self.execute_command("ovs-vsctl add-br br0", ssh=ssh)
        stdout = self.execute_command("ovs-vsctl set-manager ptcp:6640", ssh=ssh)
        #Step-5 Bring the bridge interface UP=DOWN-UP
        stdout = self.execute_command("ip link set dev br0 up", ssh=ssh)
        stdout = self.execute_command("ip link set dev br0 down", ssh=ssh)
        stdout = self.execute_command("ip link set dev br0 up", ssh=ssh)

        #Step-6 Verify if the notifications have been updated in /tmp/python-notofications.dump file
        try:
            notify_file = sftp.open(NOTIFICATION_FILE, mode='r')
        except IOError:
            self.__logger.warning(
                'Cannot open {} on node {}'.format(
                    NOTIFICATION_FILE, compute.get_name()))
            return False
        in_lines = notify_file.readlines()
        for line in in_lines:
            if 'ovs_events' in line:
                self.__logger.info('OVS Events is present in the python-notifications.dump') 
                return True
            else:
                pass
        #Step-7 remove COLLECTD_NOTIFICATION & Python notification file fromm compute node
        stdout = self.execute_command("rm -rf {}" .format(NOTIFICATION_FILE), ssh=ssh)
        
        #stdout = self.execute_command("rm -rf collectd_notification_dump.py")
        return True
    ...
    self, compute, plugins, error_plugins, create_backup=True):
        """ Checking if lugin files are available
        ...
        Return boolean value indicating whether function was successful.
        """
        self.__logger.info("sorted plugins = {}" .format(plugins))
        ssh, sftp = self.__open_sftp_session(compute.get_ip(), 'root', 'opnfvapex')
        plugins_to_enable = plugins[:]
        for plugin in plugins:
            if plugin == 'csv':
                self.__logger.info("Now enabling csv in collectd.conf file")
                is_csv_enabled = ConfigServer.enable_plugins(self, compute, plugins, error_plugins, create_backup=True)
            #elif plugin == 'python':    
                #self.__logger.info("Now enabling python in collectd.conf file")
                #is_ovs_events_enabled = ConfigServer.enable_ovs_events(self, compute, plugins, error_plugins, create_backup=False)
            else:
                plugin_file = '/usr/lib64/collectd/{}.so'.format(plugin)
                plugin_conf_file = '/etc/collectd/collectd.conf.d/{}.conf' .format(plugin)
                try:
                    sftp.stat(plugin_file)
                except IOError:
                    self.__logger.debug(
                ...
                enabled_plugins = []
                try:
                    sftp.stat(plugin_conf_file)
                except IOError:
                    self.__logger.warning(
                        '{} Plugin conf file not available in the directory' .format(plugin))
                    error_plugins.append((
                        plugin, 'plugin conf file {} not found'.format(plugin),
                        True))
                return False
                enabled_plugins.append(plugin)
        return True
    ...
    def get_gnocchi_metrics(self, criteria):
        """Login to the controller and check if the timestamps are being updated for Gnocchi.

        Keyword arguments:
        ssh_session --

        """
        def __init__(self, id_num, name, status, power_state, networks):
            self.__id_num = list() 
            self.__name = name
            ...
            self.__logger.info('Checking gnocchi metric list')
            ssh, sftp = self.__open_sftp_session(controller.get_ip(), 'root', 'opnfvapex')
            stdout = self.execute_command("source overcloudrc.v3;gnocchi metric list", controller.get_ip())
            metric_list = stdout.readlines() \

            for entry in metric_table[3:]:
                if entry[0] == '+' or entry[0] == '\n':
                    print entry
                    pass
            else: 
                 self.__id_num.append(
                 metric_table([str(x.strip( '$1')) for x in entry.split('|')]))     
            
            self.__logger.info('metric_table={}'.format(metric_table))
        #metric_ids = stdout.split(' ', 1)
