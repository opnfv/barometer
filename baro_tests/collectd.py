...
APEX_IP = functest_utils.get_parameter_from_yaml(
    'apex.ip', INSTALLER_PARAMS_YAML)
APEX_USER = functest_utils.get_parameter_from_yaml(
    'apex.user', INSTALLER_PARAMS_YAML)
APEX_PKEY = functest_utils.get_parameter_from_yaml(
    'apex.pkey', INSTALLER_PARAMS_YAML)
...
def _exec_testcase(
    ...
    if plugin_critical_errors:
        ...
        if failed_prerequisites:
            ...
            if gnocchi_running:
                res = tests.test_gnocchi_node_sends_data(
                    compute_node.get_id(),
    ...
    for compute_node in computes:
        ...
        if not conf.check_gnocchi_plugin_included(compute_node):
            ...
            if not collectd_restarted:
                ...
                gnocchi_running = (
                    gnocchi_running_on_con
                    and tests.test_gnocchi_node_sends_data(
                        'node-{}'.format(node_id),
                        10, logger, client=GnocchiClient()))
                if gnocchi_running:
        ...
        mcelog_delete()
    logger.info("Results before printing = {}" .format(results))
