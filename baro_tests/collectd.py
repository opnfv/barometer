...
INSTALLER_PARAMS_YAML = os.path.join(
    CONST.dir_repo_functest, 'functest/ci/installer_params.yaml')
FUEL_IP = functest_utils.get_parameter_from_yaml(
    'fuel.ip', INSTALLER_PARAMS_YAML)
FUEL_USER = functest_utils.get_parameter_from_yaml(
    'fuel.user', INSTALLER_PARAMS_YAML)
FUEL_PW = functest_utils.get_parameter_from_yaml(
    'fuel.password', INSTALLER_PARAMS_YAML)
...
class CeilometerClient(object):
    ...
    def _auth_server(self):
        ...
        self._logger.debug(
            'Connecting to the auth server {}'.format(os.environ['OS_AUTH_URL']))
        ...
        if self._ceilometer_url is None:
            self._logger.warning(
                'Ceilometer is not registered in service catalog')
    ...
    def _request_meters(self, criteria):
        ...
        if criteria is None:
            ...
            url = self._ceilometer_url + \
                ('/v2/meters/%s?q.field=resource_id&limit=400' % criteria)
...
class CollectdTest(testcase.TestCase):

    def _process_result(self, compute_node, test, result, results_list):
        ...
        if result:
            logger.info(
                'Compute node {0} test case {1} PASSED.'.format(compute_node, test))
        else:
            logger.error(
                'Compute node {0} test case {1} FAILED.'.format(compute_node, test))
        results_list.append((compute_node, test, result))

    def _print_label(self, label):
    ...
    def _print_plugin_label(self, plugin, node_id):
        """Print plug-in label.
        ...
        """
        self._print_label(
            'Node {0}: Plug-in {1} Test case execution'.format(node_id, plugin))

    def _print_final_result_of_plugin(self, plugin, compute_ids, results, out_plugins, out_plugin):
    ...
    def print_overall_summary(self, compute_ids, tested_plugins, results, out_plugins):
        ...
        for compute in compute_node_names:
            all_computes_in_line = all_computes_in_line + \
                '| ' + compute + (' ' * (7 - len(compute)))
        ...
        for out_plugin in out_plugins_print:
            ...
            for id in compute_ids:
                ...
                if out_plugin == 'Ceilometer':
                    out_plugin_result = 'PASS' if out_plugins[
                        id] == out_plugin else 'FAIL'
                if out_plugin == 'CSV':
                    if out_plugins[id] == out_plugin:
                        out_plugin_result = \
                            'PASS' if [
            ...
            for plugin in sorted(tested_plugins.values()):
                line_plugin = self._print_final_result_of_plugin(
                    plugin, compute_ids, results, out_plugins, out_plugin)
                logger.info(
                    '|  IN:{}'.format(plugin) + (' ' * (11 - len(plugin))) + '|' + line_plugin)
            logger.info(
                '+' + ('-' * 16) + '+' + (('-' * 8) + '+') * len(compute_node_names))
        logger.info('=' * 70)

    def _exec_testcase(
            self, test_labels, name, ceilometer_running, compute_node,
        ...
        plugin_prerequisites = {
            ...
            'ovs_events': [(
                len(ovs_existing_configured_int) > 0 or len(
                    ovs_interfaces) > 0,
        ...
        self._print_plugin_label(
            test_labels[name] if name in test_labels else name, compute_node.get_id())
            ...
            if failed_prerequisites:
                ...
                if ceilometer_running:
                    res = test_ceilometer_node_sends_data(
                        compute_node.get_id(), conf.get_plugin_interval(
                            compute_node, name),
                    ...
                    res = test_csv_handles_plugin_data(
                        compute_node, conf.get_plugin_interval(
    ...
    def mcelog_install(self):
        """Install mcelog on compute nodes.

        Keyword arguments:
        logger - logger instance
        """
        self._print_label('Enabling mcelog on compute nodes')
        handler = factory.Factory.get_handler(
            'fuel', FUEL_IP, FUEL_USER, installer_pwd='')
                ...
                if node.is_compute():
                    ...
                    if '16.04' not in ubuntu_release:
                        ...
                        if 'mce-inject_df' and 'corrected' in res:
                            ...
                            res = node.run_cmd(
                                '/root/mce-inject_df < /root/corrected')
                        else:
                            ...
                            res = node.run_cmd(
                                'echo "CPU 0 BANK 0" > /root/corrected')
                            res = node.run_cmd(
                                'echo "STATUS 0xcc00008000010090" >> /root/corrected')
                            res = node.run_cmd(
                                'echo "ADDR 0x0010FFFFFFF" >> /root/corrected')
                            res = node.run_cmd('modprobe mce-inject')
                            res = node.run_cmd(
                                '/root/mce-inject_df < /root/corrected')
            logger.info('Mcelog is installed on all compute nodes')

    def mcelog_delete(self):
    ...
    def get_ssh_keys(self):
        ...
        if not os.path.isfile(ID_RSA_DST):
            logger.info(
                "RSA key file {} doesn't exist, it will be downloaded from installer node.".format(ID_RSA_DST))
            handler = factory.Factory.get_handler(
                'fuel', FUEL_IP, FUEL_USER, installer_pwd=FUEL_PW)
    ...
    def run(self):
        ...
        self.mcelog_install()  # installation of mcelog

        ceilometer_running_on_con = False
        self._print_label('Test Ceilometer on control nodes')
        ...
        for compute_node in computes:
            ...
            plugins_to_enable = []
            self._print_label(
                'NODE {}: Test Ceilometer Plug-in'.format(node_id))
            ...
            if not conf.check_ceil_plugin_included(compute_node):
                ...
                logger.info(
                    'Testcases on node {} will not be executed'.format(node_id))
            else:
                collectd_restarted, collectd_warnings = conf.restart_collectd(
                    compute_node)
                sleep_time = 30
                logger.info(
                ...
                if not collectd_restarted:
                    ...
                    logger.error(
                        'Restart of collectd on node {} failed'.format(node_id))
                    logger.info(
                        'Testcases on node {} will not be executed'.format(node_id))
                else:
                    for warning in collectd_warnings:
                    ...
                    if ceilometer_running:
                        ...
                        logger.info(
                            "CSV will be enabled for verification of test plugins.")
                ...
                self._print_label(
                    'NODE {}: Restoring config file'.format(node_id))
                conf.restore_config(compute_node)

        self.mcelog_delete()  # uninstalling mcelog from compute nodes

        self.print_overall_summary(
            compute_ids, plugin_labels, results, out_plugins)
...
if __name__ == '__main__':
    ...
    BarometerTests = CollectdTest()
    sys.exit(BarometerTests.run())
