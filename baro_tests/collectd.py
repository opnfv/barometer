...
INSTALLER_TYPE = functest_utils.get_installer_type()
...
APEX_IP = functest_utils.get_parameter_from_yaml('apex.ip', INSTALLER_PARAMS_YAML)
APEX_USER = functest_utils.get_parameter_from_yaml('apex.user', INSTALLER_PARAMS_YAML)
APEX_PKEY = functest_utils.get_parameter_from_yaml('apex.pkey', INSTALLER_PARAMS_YAML)
...
class CeilometerClient(object):
    ...
    def _auth_server(self):
        ...
        if self._ceilometer_url is None:
            LOGGER.warning('Ceilometer is not registered in service catalog')

    def _request_meters(self, criteria):
        """Request meter list values from ceilometer Keyword arguments: criteria -- criteria for ceilometer meter list """
        if criteria is None:
    ...
    def get_csv_metrics(
        ...
        for plugin_subdir in plugin_subdirectories:
            for meter_category in meter_categories:
                stdout = self.conf.execute_command(
                    "tail -2 /var/lib/collectd/csv/{0}.jf.intel.com/{1}/{2}-{3}".format(
...
def _exec_testcase(
    ...
    if plugin_critical_errors:
        ...
        if failed_prerequisites:
            ...
            if gnocchi_running:
                res = test_gnocchi_node_sends_data(
                    compute_node.get_id(),
                    conf.get_plugin_interval(compute_node, name),
                    logger=LOGGER, client=GnocchiClient(),
...
def get_results_for_ovs_events(plugin_labels, plugin_name,
        gnocchi_running, compute_node,
        conf, results, error_plugins):
    """ Testing OVS Events with python plugin 
    """
    plugin_label = 'OVS events'
    res = conf.enable_ovs_events(compute_node, plugin_label, error_plugins, create_backup=False)
    _process_result(
        ...
        if node.is_compute():
            ...
            if '3.10.0-514.26.2.el7.x86_64' not in centos_release:
                LOGGER.info(
                    ...
                    + 'on node-{0}, '.format(node.get_name())
                    + 'unsupported Ubuntu release found ({1}).'.format(
                        centos_release))
        else:
            LOGGER.info(
            ...
            if 'mce-inject_ea' and 'corrected' in res:
                ...
                node.run_cmd('/root/mce-inject_ea < /root/corrected')
            else:
                LOGGER.info(
                ...
                node.put_file(
                    ...
                    + 'mce-inject_ea', '/root/mce-inject_ea')
                node.run_cmd('chmod a+x /root/mce-inject_ea')
                node.run_cmd('echo "CPU 0 BANK 0" > /root/corrected')
                node.run_cmd(
    ...
    for node in nodes:
        if node.is_compute():
            ...
            if 'mce-inject_ea' in output:
                node.run_cmd('rm /root/mce-inject_df')
            if 'corrected' in output:
                node.run_cmd('rm /root/corrected')
...
def main(bt_logger=None):
    ...
    get_ssh_keys()
    conf = ConfigServer(APEX_IP, APEX_USER, LOGGER)
    ...
    for compute_node in computes:
        ...
        if not conf.check_gnocchi_plugin_included(compute_node):
            ...
            if not collectd_restarted:
                ...
                gnocchi_running = (
                    gnocchi_running_on_con
                    and test_gnocchi_node_sends_data(
                        'node-{}'.format(node_id), 
                        10, logger=LOGGER, client=GnocchiClient()))
                if gnocchi_running:
                    out_plugins[node_id] = 'Gnocchi'
                    LOGGER.info("Gnocchi is running.")
                else:
                    plugins_to_enable.append('csv')
                    out_plugins[node_id] = 'CSV'
                    LOGGER.error("Gnocchi is not running.")
                    LOGGER.info(
